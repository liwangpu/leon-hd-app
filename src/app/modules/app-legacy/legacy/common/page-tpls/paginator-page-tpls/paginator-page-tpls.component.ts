...
export class PaginatorPageTplsComponent implements OnInit {
  ...
  ngOnInit() {
    //转移launch参数到mdSrv
    this.icon = this.launch.titleIcon;
    this.title = this.launch.title;
    ...
    this.mdSrv.readDataOnly = this.launch.readDataOnly;
    this.mdSrv.pagedModel = this.launch.pagedModel;
    // if (!this.navSrv.isLatestVisitPage(this.router.snapshot.url))
    this.mdSrv.displayMode = this.mdSrv.pagedModel[0];
